...
import { getExecutionByID } from '../db/executions';

const VALID_LOG_LEVELS: ExecutionLog['logLevel'][] = [
  'debug',
  'info',
  'warn',
  'error',
...
export async function handlerCreateExecutionLog(
  config: ApiConfig,
  ...
  const execution = await getExecutionByID(config.db, body.executionID);
  ...
  const params: CreateExecutionLogParams = {
    ...
    contextJSON: JSON.stringify(body.contextJSON) ?? null,
  };
...
export async function handlerListExecutionLogs(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
  ...
  if (executionID) {
    ...
    throw new BadRequestError(
      'executionID or logLevel query parameter is required',
    );
  }
  return respondWithJSON(200, logs);
}

export async function handlerGetExecutionLogByID(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
  ...
  const hasOwnership = await verifyExecutionLogOwnership(
    config.db,
    logID,
    userID,
  );
...
export async function handlerDeleteExecutionLog(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
...
export async function handlerDeleteExecutionLogsByExecutionID(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
  ...
  const deletedCount = await deleteExecutionLogsByExecutionID(
    config.db,
    executionID,
    userID,
  );
