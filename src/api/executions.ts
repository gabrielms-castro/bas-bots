...
export async function handlerCreateExecution(config: ApiConfig, req: AuthenticatedRequest): Promise<Response> {
...
export async function handlerListAllExecutions(config: ApiConfig,req: AuthenticatedRequest): Promise<Response> {
    const userID = req.user.id;
    ...
    let executions;
    if (robotInstanceID) {
        executions = await listExecutionsByRobotInstanceID(config.db, userID, robotInstanceID);
    } else if (status) {
...
export async function handlerGetExecutionByID(config: ApiConfig, req: AuthenticatedRequest): Promise<Response> {
    const userID = req.user.id;
    const url = new URL(req.url);
...
export async function handlerUpdateExecution(config: ApiConfig, req: AuthenticatedRequest): Promise<Response> {
    const userID = req.user.id;
    const url = new URL(req.url);
    const pathParts = url.pathname.split("/");
    const executionID = pathParts[pathParts.length - 1];
    ...
    if (body.finishedAt !== undefined) params.finishedAt = new Date(body.finishedAt);
    ...
    if (body.outputDataJson !== undefined) params.outputDataJson = body.outputDataJson;
    if (body.outputFilePath !== undefined) params.outputFilePath = body.outputFilePath;
    ...
    if (body.durationSeconds !== undefined) params.durationSeconds = body.durationSeconds;

    const updatedExecution = await updateExecution(config.db, executionID, userID, params);

    return respondWithJSON(200, updatedExecution);
}

export async function handlerDeleteExecution(config: ApiConfig,req: AuthenticatedRequest): Promise<Response> {
    const userID = req.user.id;
    const url = new URL(req.url);
    const pathParts = url.pathname.split("/");
    const executionID = pathParts[pathParts.length - 1];
