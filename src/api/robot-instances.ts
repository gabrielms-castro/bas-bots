...
export async function handlerCreateRobotInstance(
  config: ApiConfig,
  ...
  if (hasCredential === hasExtension) {
    throw new BadRequestError(
      'Exactly one of credentialID or extensionID must be provided',
    );
  }
...
export async function handlerListAllRobotInstancesByUserID(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
...
export async function handlerGetRobotInstanceByID(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
...
export async function handlerUpdateRobotInstance(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
  ...
  const newExtensionID =
    body.extensionID !== undefined ? body.extensionID : existing.extensionID;
  ...
  if (hasCredential === hasExtension) {
    throw new BadRequestError(
      'Exactly one of credentialID or extensionID must be set',
    );
  }
  ...
  if (body.parametersJSON !== undefined)
    params.parametersJSON = body.parametersJSON;

  const updated = await updateRobotInstance(
    config.db,
    instanceID,
    userID,
    params,
...
export async function handlerDeleteRobotInstance(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
  ...
  if (!deleted)
    throw new InternalServerError('Failed to delete robot instance');
