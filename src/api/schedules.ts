...
export async function handlerCreateSchedule(
  config: ApiConfig,
  ...
  if (!body.robotInstanceID)
    throw new BadRequestError('robotInstanceID is required');
  ...
  const params: CreateScheduleParams = {
    ...
    lastExecutionAt: body.lastExecutionAt
      ? new Date(body.lastExecutionAt)
      : null,
    nextExecutionAt: body.nextExecutionAt
      ? new Date(body.nextExecutionAt)
      : null,
...
export async function handlerListAllSchedules(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
...
export async function handlerGetScheduleByID(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
...
export async function handlerLDeleteSchedule(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
...
export async function handlerUpdateSchedule(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
  ...
  if (body.cronExpression !== undefined)
    params.cronExpression = body.cronExpression;
  if (body.timezone !== undefined) params.timezone = body.timezone;
  if (body.isActive !== undefined) params.isActive = !!body.isActive;
  if (body.lastExecutionAt !== undefined)
    params.lastExecutionAt = body.lastExecutionAt
      ? new Date(body.lastExecutionAt)
      : null;
  if (body.nextExecutionAt !== undefined)
    params.nextExecutionAt = body.nextExecutionAt
      ? new Date(body.nextExecutionAt)
      : null;
  ...
  if (body.retryDelayMinutes !== undefined)
    params.retryDelayMinutes = body.retryDelayMinutes;
