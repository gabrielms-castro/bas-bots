...
export async function handlerCreateRobot(
  config: ApiConfig,
  ...
  if (
    ...
    throw new BadRequestError(
      'robotName, robotType, description and sourceFilePath are required',
    );
  }
...
export async function handlerListAllRobots(
  config: ApiConfig,
...
export async function handlerGetRobotByID(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
...
export async function handlerLDeleteRobot(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const url = new URL(req.url);
...
export async function handlerUpdateRobot(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const url = new URL(req.url);
  ...
  if (body.requiresCredential !== undefined)
    params.requiresCredential = !!body.requiresCredential;
  if (body.requiresExtension !== undefined)
    params.requiresExtension = !!body.requiresExtension;
  if (body.supportedExtensions !== undefined)
    params.supportedExtensions = body.supportedExtensions;
  if (body.sourceFilePath !== undefined)
    params.sourceFilePath = body.sourceFilePath;
  if (body.parametersSchema !== undefined)
    params.parametersSchema = body.parametersSchema;
