...
export async function handlerCreateExtension(
  config: ApiConfig,
  ...
  if (!body.extensionName)
    throw new BadRequestError('extensionName is required');
  ...
  const existing = await getExtensionByName(
    config.db,
    userID,
    body.extensionName,
  );
  if (existing)
    throw new BadRequestError('Extension with this name already exists');
...
export async function handlerListExtensions(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
...
export async function handlerGetExtensionByID(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
...
export async function handlerUpdateExtension(
  ...
  if (body.extensionName && body.extensionName !== existing.extensionName) {
    const conflict = await getExtensionByName(
      config.db,
      userID,
      body.extensionName,
    );
  ...
  if (body.extensionName !== undefined)
    params.extensionName = body.extensionName;
...
export async function handlerDeleteExtension(
  config: ApiConfig,
  req: AuthenticatedRequest,
) {
  const userID = req.user.id;
