...
export async function createExecutionLog(db: Database, params: CreateExecutionLogParams): Promise<ExecutionLog | undefined> {
    const id = randomUUID();
    const sql = `
...
export async function listExecutionLogsByExecutionID(db: Database, executionID: string, userID: string): Promise<ExecutionLog[]> {
    const sql = `
        SELECT el.*
...
export async function listExecutionLogsByLevel(db: Database, userID: string, logLevel: ExecutionLog["logLevel"]): Promise<ExecutionLog[]> {
  const sql = `
    SELECT el.*
    FROM execution_logs el
    INNER JOIN executions e ON el.execution_id = e.id
...
export async function getExecutionLogByID(db: Database, id: string): Promise<ExecutionLog | undefined> {
    const sql = `
        SELECT *
        FROM execution_logs
        WHERE id = ?
...
export async function verifyExecutionLogOwnership(db: Database, logID: string, userID: string): Promise<boolean> {
    const sql = `
        SELECT 1
        FROM execution_logs el
        INNER JOIN executions e ON el.execution_id = e.id
    ...
    `;
    const result = db.query<{ "1": number }, [string, string]>(sql).get(logID, userID);
    return !!result;
}

export async function deleteExecutionLog(db: Database, id: string, userID: string): Promise<boolean> {
    const sql = `
...
export async function deleteExecutionLogsByExecutionID(db: Database, executionID: string, userID: string): Promise<number> {
    const sql = `
        DELETE FROM execution_logs
