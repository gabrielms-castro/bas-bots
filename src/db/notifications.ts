...
export async function listUnreadNotificationsByUserID(
  db: Database,
  userID: string,
): Promise<Notification[]> {
  const sql = `
...
export async function listNotificationsByType(
  ...
  const rows = db
    .prepare(sql)
    .all(userID, notificationType) as NotificationRow[];
  return rows.map(rowToNotification);
}

export async function listNotificationsByExecutionID(
  db: Database,
  userID: string,
  executionID: string,
): Promise<Notification[]> {
