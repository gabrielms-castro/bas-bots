...
export async function createExtension(db: Database, params: CreateExtensionParams): Promise<Extension | undefined> {
    const id = randomUUID();
    const sql = `
...
export async function listExtensionsByUserID(db: Database, userID: string): Promise<Extension[]> {
    const sql = `
        SELECT *
        FROM extensions
        WHERE user_id = ?
...
export async function getExtensionByID(db: Database, id: string): Promise<Extension | undefined> {
    const sql = `
        SELECT *
        FROM extensions
        WHERE id = ?
...
export async function getExtensionByName(db: Database, userID: string, extensionName: string): Promise<Extension | undefined> {
    const sql = `
        SELECT *
        FROM extensions
        WHERE user_id = ? AND extension_name = ?
    `;
    const row = db.query<ExtensionRow, [string, string]>(sql).get(userID, extensionName);
    if (!row) return undefined;
    return rowToExtension(row);
}

export async function updateExtension(db: Database, id: string, userID: string, params: UpdateExtensionParams): Promise<Extension | undefined> {
...
export async function deleteExtension(db: Database, id: string, userID: string): Promise<boolean> {
    const sql = `
