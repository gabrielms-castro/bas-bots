...
export async function createRobot(db: Database, params: CreateRobotParams): Promise<Robot | undefined> {
    const id = randomUUID();
    const sql = `
        INSERT INTO robots (
            id,
...
export async function getRobotByID(db: Database, id: string): Promise<Robot | undefined> {
    const sql = `
        SELECT *
        FROM robots
        WHERE id = ?
...
export async function updateRobot(db: Database, id: string, params: UpdateRobotParams): Promise<Robot | undefined> {
